...
struct FixedEffect{R <: AbstractVector{<:Integer}, I <: AbstractVector{<: Real}, U}
    ...
    function FixedEffect{R, I, U}(refs, interaction, n) where {R <: AbstractVector{<:Integer}, I <: AbstractVector{<: Real}, U <: Union{Val{false}, Val{true})}
...
function FixedEffect(args...; interaction::AbstractVector = Ones{Float64}(length(args[1])), crossing = false)
    if length(args) != 1
        ...
        if !isa(x, CategoricalVector)
            ...
            FixedEffect{typeof(x.refs), typeof(interaction), Val{crossing})}(x.refs, interaction, length(x.pool))
        end
    end
end
...
function Base.show(io::IO, fe::FixedEffect)
    ...
    println(io, "Crossing:    ", has_crossing(fe))

end

Base.ismissing(fe::FixedEffect) = any(fe.refs .== 0)  | ismissing(fe.interaction)
has_crossing(fe::FixedEffect) = false
has_crossing(fe::FixedEffect{R, I, Val{true}}) = true
##############################################################################
##
